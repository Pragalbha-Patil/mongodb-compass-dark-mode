import { ReactElement } from 'react';
import PropTypes from 'prop-types';
import { LogoProps, SupportedColors } from './utils';
/**
 * # EnterpriseAdvancedLogoLockup
 *
 * React Component that displays the Atlas Logo Lockup.
 *
 * ```
 * <EnterpriseAdvancedLogoLockup />
 * ```
 * @param props.color Determines the color of the logo.
 * @param props.height Determines height of the <Logo /> component.
 */
declare function EnterpriseAdvancedLogoLockup({ height, className, color, role, 'aria-label': ariaLabel, ...rest }: LogoProps): ReactElement;
declare namespace EnterpriseAdvancedLogoLockup {
    var displayName: string;
    var propTypes: {
        height: PropTypes.Requireable<number>;
        color: PropTypes.Requireable<SupportedColors>;
    };
}
export default EnterpriseAdvancedLogoLockup;
//# sourceMappingURL=EnterpriseAdvancedLogoLockup.d.ts.map