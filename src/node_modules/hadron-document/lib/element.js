...
var DateEditor = require('./editor/date');
...
var UNEDITABLE_TYPES = ['Binary', 'Code', 'MinKey', 'MaxKey', 'Timestamp', 'BSONRegExp', 'Undefined', 'Null', 'DBRef'];
/**
  ...
  _createClass(Element, [{
    ...
    value: function changeType(newType) {
      if (newType === 'Object') {
        ...
        try {
          if (newType === 'Date') {
            var editor = new DateEditor(this);
    ...
    key: "insertSiblingPlaceholder",
    value: function insertSiblingPlaceholder() {
    ...
    value: function isDuplicateKey(value) {
      ...
      var _iterator = _createForOfIteratorHelper(this.parent.elements),
          _step;
    ...
    key: "_bubbleUp",
    value: function _bubbleUp(evt) {
      for (var _len = arguments.length, data = new Array(_len > 1 ? _len - 1 : 0), _key2 = 1; _key2 < _len; _key2++) {
        data[_key2 - 1] = arguments[_key2];
      }

      this.emit.apply(this, [evt].concat(data));
      var element = this.parent;

      if (element) {
        if (element.isRoot()) {
          element.emit.apply(element, [evt].concat(data));
        } else {
          element._bubbleUp.apply(element, [evt].concat(data));
        }
      }
    }
    /**
